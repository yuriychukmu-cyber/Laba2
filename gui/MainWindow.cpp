...
void MainWindow::setupPart1Block(QVBoxLayout *rootLayout)
{
    ...
    auto *popButton = new QPushButton("Удалить первый", this);
    ...
    connect(popButton, &QPushButton::clicked, this, &MainWindow::popPart1Item);
...
void MainWindow::setupPart2Block(QVBoxLayout *rootLayout)
{
    ...
    connect(popButton, &QPushButton::clicked, this, &MainWindow::popPart2Item);
    connect(showButton, &QPushButton::clicked, this, &MainWindow::showPart2Queue);
    connect(m_part2Type, &QComboBox::currentTextChanged, this, [this]() {
        refreshPart2Output();
    });
...
void MainWindow::popPart1Item()
{
    try {
        const QueueItem removed = m_part1Queue.pop();
        setPart1Status(QString("Удалён элемент: %1").arg(removed.toString()), false);
        refreshPart1Output();
    } catch (const QueueException &ex) {
        setPart1Status(ex.what(), true);
    }
...
void MainWindow::popPart2Item()
{
    try {
        if (m_part2Type->currentText() == "int") {
            const int removed = m_intQueue.pop();
            m_part2Status->setText(QString("Удалён из TemplateQueue<int>: %1").arg(removed));
        } else {
            const QString removed = m_stringQueue.pop();
            m_part2Status->setText(QString("Удалён из TemplateQueue<QString>: %1").arg(removed));
        }
        m_part2Status->setStyleSheet("color: #1b5e20;");
        refreshPart2Output();
    } catch (const TemplateQueueEmptyException &ex) {
        m_part2Status->setText(ex.what());
        m_part2Status->setStyleSheet("color: #b71c1c;");
    }
}
...
void MainWindow::refreshPart1Output()
{
    m_part1Output->setPlainText(m_part1Queue.toString());
}

void MainWindow::refreshPart2Output()
{
    if (m_part2Type->currentText() == "int") {
